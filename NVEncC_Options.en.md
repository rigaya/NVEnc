...
the height must be an even value of at least 4. The model must use the half-height,
6-channel restoration output generated by the current `export_stdeint.py`.
Legacy models with ONNX weave output must be re-exported.
...
The CUDA device path uses zero-copy device tensors. If initialization or execution fails,
the filter falls back to the host path. The selected path is shown as `path cuda-zerocopy`
or `path host` in the log.
