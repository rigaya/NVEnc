...
#include <platform/include/NvCompiler.h>
#include "xcodeutil.h"
...
#define SYMVER , "GLIBC_" SYMVER_LINUX_ARM
#else
#error "SYMVER undefined for this architecture."
#endif
#else // !linux
#define DLSYM dlsym
#define SYMVER
...
#define GET_PROC(proc) {                                            \
        _nv_##proc =                                                \
                                                                    (typeof(_nv_##proc))DLSYM(dlopen_handle, #proc SYMVER); \
        if (_nv_##proc == 0) {                                      \
            TPRINTF(("%s: Error detected\n", __NVFUNCTION__));      \
