...
ONNX Runtime CUDA/TensorRT provider を使用してONNXモデルを実行するCNNフィルタ。Windows x64向けで、CUDA 12対応のONNX Runtime GPUビルドが必要。

- **パラメータ**
  - model=&lt;string&gt;
    ONNXモデルファイルのパス。
  - provider=&lt;string&gt; (デフォルト: cuda)
    cuda または tensorrt。
  - out_res=&lt;int&gt;x&lt;int&gt;, resize=&lt;string&gt;
    モデル実行後に指定解像度へリサイズする。
