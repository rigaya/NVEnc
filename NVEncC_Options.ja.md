...
使用する場合は、追加パッケージ [NVEncNVVFXDenoise_20260925_x64.7z](https://github.com/rigaya/NVEnc/releases/download/9.35/NVEncNVVFXDenoise_20260925_x64.7z) をダウンロードし、NVEncCの実行ファイルと同じ場所に展開する必要がある。
...
- **オプション**
  - algo=&lt;string&gt;  
    ...
    - [NGX](https://docs.nvidia.com/rtx/ngx/programming-guide/index.html)ライブラリのリサイズフィルタ
      ...
      - 追加パラメータ
        - vsr-quality=&lt;int&gt;  
          ngx-vsrで使用するモデル/モードの選択。 (NVIDIAのパラメータ名は "QualityLevel"、デフォルト=1, 0, 1 - 4, 8 - 19, 21, 23)
          - 0 ... バイキュービックのベースライン (非AI、比較用ベースラインとして有用)
          - 1 - 4 ... VSR 品質レベル
          - 8 - 11 ... デノイズ
          - 12 - 15 ... デブラー (ブラー除去)
          - 16 - 19 ... 高ビットレート向けのディテール復元
          - 21 ... ストリーミング (medium)
          - 23 ... ストリーミング (ultra)
          1 - 4, 16 - 19, 21, 23 は数字が大きいほど高品質、8 - 15 は数字が大きいほど効果が強い。
          モード 0 と 8 - 19 は VFX SDK 1.2 以降の nvngx_vsr.dll が必要。
          モード 21 と 23 は VFX SDK 1.3 以降の nvngx_vsr.dll と Ampere 世代以降の GPU が必要。
          モード 5 - 7, 20, 22 はNVIDIAにより予約されており、使用できない。
          モード 8 - 15 は解像度を変更しないので、出力解像度を入力解像度と同じにする必要がある。
        - vsr-strength=&lt;float&gt;  
          ngx-vsr使用時の強さの指定。 (0.0 - 1.0, デフォルト = 1.0)
          数字が大きいほど強く適用される。
          VFX SDK 1.3 以降の nvngx_vsr.dll が必要。(1.2 の dll では無視される)
